...
class DoublyLinkedList:
    ...
    #     previous_node = None
        ...
        while current_node.get.value() != number: 
            ...
            if self.prev: 
                self.prev.next_node(current_node.get_next.node)
                self.prev = None
...
def add_person(self, person):
    add_to = input("Who would you like to add? ")
    while add_to == True:
            self.queue.append(person)
            break

def remove_person(self, delete):
    delete_from = input("Who would you like to remove? ")
    while delete_from == True:
        self.queue.popleft()
        break
